...
<!--  script src="/Questionaire/web/Qdisplay/html5/js/jquery.mobile-1.4.5.js"></script>-->
<!-- 问题选项显示方式 -->
...
<!-- 收集问卷答案，将答案发送到后台的js -->
<script type="text/javascript"
	src="/Questionaire/web/Qdisplay/html5/js/answerCollectionMobile.js"></script>
...
<!-- 实现上一页按钮功能的js -->
<script type="text/javascript" src="/Questionaire/web/Qdisplay/html5/js/prePageFunction.js"></script>

<!-- 实现下一页按钮功能的js -->
<script type="text/javascript" src="/Questionaire/web/Qdisplay/html5/js/nextPageFunction.js"></script>
...
<!--实现移动端 逻辑-->
<script type="text/javascript" src="/Questionaire/web/Qdisplay/html5/js/logicMobile.js"></script>
...
<body>
	<%
		//实例化一个辅助对象，利用该对象可以获取spring中的bean
		    ...
		    QueDisplayService service = (QueDisplayService)wac.getBean("QueDisplayService");
		
		    //获取问卷的id
		    Integer qnaireId = null;
		    if(request.getAttribute("qnaireId")!=null){
		    	
		    	qnaireId =Integer.valueOf(request.getAttribute("qnaireId").toString());
		    
		    }else if(request.getParameter("qnaireId")!=null){
		    	
		    	qnaireId =Integer.valueOf(request.getParameter("qnaireId"));
		    }
		    ...
		    //获取该问卷的所有问题
		    ArrayList<QuestionBasicinfo> questions = service.getQuestionByQnaireId(qnaireId);
	...
	<div  class="view main" id="<%=qnaireId%>" data-theme="a"
		style="background-color:#F8F8FF;">
		
		
			<!------------------------ 页头，展示问卷的一些基本信息，比如问卷标题等 --------------------------------->
			<!-- 此处设置header的颜色的css代码只能用内联样式，不然会被jquerymobile的theme给覆盖 -->
			...
			<div  id="pageContent" class="pages qnaireContent">
				...
				<!-- ----------------------------显示问题说明以及问题logo --------------------------------- -->
				<div data-role="page" class="page"  ifDisplay="1"  id="page<%=pageNum%>"data-id="<%=pageNum%>">
				<!------------------------ 页头，展示问卷的一些基本信息，比如问卷标题等 ------------------------------- -->
...
<!------------------------- 矩阵单选题 ------------------------------------------------------------>
								...
								double rowNum=0;
								  ...
								  <table id="<%=question.getQuesBasicInfoId()%>-head" class="topBox" cellspacing="0">
                                   ...
                                   <% 
											...
											int g=1;
											int colNum_page=0;//为矩阵每五行添加一个表头设置参数
											while (it2.hasNext()) {
                								colNum_page++;
								...
								<tr class="rowOption" id="<%=rowTemp.getOptionBasicInfoId()%>">
									...
									<%
										//对列选项进行遍历
										   ...
										   %>
									 <td data-id="op<%=matrixRatingNum %><%=g%>-<%=d%>" class="matrix-rating-td">
										<%
											}else{
											  %>
										...
										%> 
			      						...
			      						<input type="radio" name="<%=rowTemp.getOptionBasicInfoId()%>" id="<%=rowTemp.getOptionBasicInfoId()%>-<%=colTemp.getOptionBasicInfoId()%>" 
                               	...
                               	<%for(int f=1;f<=colNumOp;f++){ %>
                               	    ...
                               	    <tr class="op<%=matrixRatingNum %><%=g%>"style="display:none" id="op<%=matrixRatingNum %><%=g%>-<%=f%>">
								...
								double rowNums=0;
								  ...
								  <table id="<%=question.getQuesBasicInfoId()%>-head" class="topBox" cellspacing="0">
                                   ...
                                   <% 
											...
											Iterator it2 = rowOptions.iterator();
											int gs=1;
											int colNum_page_check=0;//为矩阵每五行添加一个表头设置
											while (it2.hasNext()) {
												colNum_page_check++;
												OptionBasicinfo rowTemp = (OptionBasicinfo) it2.next();
							
								 //为矩阵每五行添加一个表头if函数
                               if(colNum_page_check!=1){
                               if(colNum_page_check%5==1){
                            	%>
                            	<tr class="colOptionTr colOptionTitlePage">
								...
								<tr class="rowOption" id="<%=rowTemp.getOptionBasicInfoId()%>">
									...
									if (colTemp.getIfFillBlank().intValue() == 1) {
										   %>
									 <td data-id="ops<%=matrixRatingNum%><%=gs%>-<%=ds%>" class="matrix-rating-td">
										<%
											}else{
											  %>
									...
									<%for(int f=1;f<=colNumOps;f++){ %>
                               	    ...
                               	    <tr class="ops<%=matrixRatingNum%><%=gs%>"style="display:none" id="ops<%=matrixRatingNum %><%=gs%>-<%=f%>">
						...
						<!-- 此处id为问题的id -->
						<div class="Question Q_type20" id="<%=question.getQuesBasicInfoId()%>">
				...
				<!-- 隐藏分页中表头-->
					...
					<%
							...
							<div class="QuestionContent">
								<ul class="unstyled M_sorting" maxOptionNum="<%=qattr.getMaxOptionNum()%>">
									...
									<li class="unsort Option" id="<%=optionId%>">
										...
										<!-- 选项标题 --> 
										<span class="OptionTitle" style="padding:5%"><%=option.getOptionTitle()%></span>
						...
						<div class="Question Q_type24 sliderQuestion" data-role="fieldcontain"
							id="<%=question.getQuesBasicInfoId()%>"
							ifquesrequired="<%=question.getQuesRequired()%>"
							ifRowRan="<%=question.getIfRowRan()%>"
							scaleStyleType="3"
						...
						%>
							...
							<div class="main-fd clearfix">
								...
								%>
							<div class="submitD" style="display:inline;float:right;margin-right:6%;width:40%;margin-top:10px">
							<div class="submit qnaireSubmit" style="margin:auto;">提交</div>
					<%
						if(request.getAttribute("IfSubmitCode").equals("1")){
				
					%>
					<div class="submitCode" style="float:right">
        ...
        <!-------------------------问卷页脚 ------------------------------------------------------>
				<footer class="qnaireFooter">

					<h3 class="qnaireFooter_text">大数据调研平台 技术支持</h3>
